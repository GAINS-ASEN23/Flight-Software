...
void loop() {
	...
	u_int32_t thrust_counter = 0;
	float thrust_start = 0.0;				// Time of the thrust start
	float thrust_avg = 0.0;					// The average thrust each sample period
	...
	// Set the pointers for ground update and det input
	KF.set_mea_input_vector(z_n);
	KF.set_det_input_vector(u_n);

	// Delay the starting of the KF (DO NOT REMOVE, weird stuff happen...)
	delay(2000);
    ...
    while(true)
    {
		...
		if (thrusting == true)
		{
			...
			thrust_avg = thrust_avg/(float)thrust_counter;
		...
		// Get the time at the end of sampling
		t2 = (millis()/1000.0) - t0;
