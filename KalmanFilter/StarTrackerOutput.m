...
t = 1:dt:60*20;                                                                                       % Create time Vector of Spacecraft Orbit
...
for n = 1:length(t)                                                                                     % Run ODE45 Simulations
    ...
    r_sc = (propagateState(newOE,t(n),t_0, MU))';
    
    % Create Hill Frame
    % NOTE: Circular Orbit Velocity is CONSTANT-ish
    % Hill Frame: {Or, Otheta, Oh}
    Or = [-r_sc(1)/norm(r_sc(1:3)) r_sc(2)/norm(r_sc(1:3)) r_sc(3)/norm(r_sc(1:3))];
    Oh = (cross(r_sc(1:3),r_sc(4:6)))./norm(cross(r_sc(1:3),r_sc(4:6)));
    Otheta = cross(Oh(:), Or(:));
    
    % Create Rotation Matrix
    R = [Or(:)'; Otheta(:)'; Oh(:)'];                                                                    % Rotation Matrix - Hill -> ECI
    
    % Calculate Quaternion with Rotation Matrix
    quat(n,:) = rotm2quat(R);                                                                            % Quaternions from Rotation Matrix
    r_scOut(n,:) = r_sc;
end

%% CSV Output
Output = [r_scOut(:,:), quat(:,:)];
writematrix(Output,'StarTrackerSample2.csv')
toc
